...
;; (pretty-print (expand (expand (expand '(-define-external-callback s_body_callback       ((c-pointer "cpBody") object))))))
...
;; C_word s_post_solve_callback(cpArbiter* arb, cpSpace* space  , C_word userdata) ;

;; void n_post_solve_callback( cpArbiter* arb, cpSpace* space, void *data) {
;;   C_word * d = (C_word*)data;
;;   *d = s_post_solve_callback(arb,space, *) ;
;; }

;; <#

;; ;; wrap scheme procedure as callback
;; (define (my-post-solve arbiter space userdata)
;;   (do-my-thing)
;;   true ;; wrapped should convert to cpTrue or cpFalse
;;   )

;; (define (wrap-post-solve proc)


;;   )


;; (let ([h (cp:space-add-collision-handler space)]
;;       [post-solve-ptr (wrap-post-solve-func my-post-solve)])
;;   (set! (collision-handler-post-solve-func h) post-solve-ptr))
